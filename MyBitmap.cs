...
namespace ProjektASM
{
    internal static class MyBitmap
    {
                                    ...
                                    byte greenLevel, byte redLevel,int numberOfThreads)
        {
            ...
            {
                int pixelIndex = k * 4;
                blue = (float)Math.Round(255.0f / blueLevelFloat * (float)pixelBuffer[pixelIndex]);
                green = (float)Math.Round(255.0f / greenLevelFloat * (float)pixelBuffer[pixelIndex + 1]);
                red = (float)Math.Round(255.0f / redLevelFloat * (float)pixelBuffer[pixelIndex + 2]);
